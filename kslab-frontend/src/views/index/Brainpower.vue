<template>
    <div class="width_1200 index_car" style="margin-top:40px;">
        <div class="car_titile_header">
            <div>
              <i></i>
              <span>智能设备</span>
              <span class="brainpower_title_font">SMART DEVICE</span>
            </div>
        </div>
        <el-tabs v-model="activeName" @tab-click="handleClick" >
              ...
              <div class="car_pro_section_body pro">
                  <ul>
                      <li v-for="(questionsList ,index) in questions" :key="index">
                         <p class="car_pro_font" @click="RouterIndexAnwser(index)">{{questionsList.title}}</p>
                          <key-wrod :keyword="['精品教程','精品教程1']"></key-wrod>
                         <div class="car_pro_Dz">
                            <div class="">
                                <ul class="car_disFlx" @click="RouterIndexAnwser(index)">
                                   <li>
                                     <img style="width:8px;" src="/static/img/icon/dc.png" alt="">
                                     <span>{{questionsList.like_num}}</span>
                                   </li>
                                   <li>
                                     <img src="/static/img/icon/private.png" alt="">
                                     <span>{{questionsList.comment_num}}</span>
                                   </li>
                                   <li>
                                     <img style="width:18px;top:0" src="/static/img/icon/yeai.png" alt="">
            ...
            <el-tab-pane name="third">
              ...
              <div class="car_Art_section_body Article">
                  <ul>
                    <li v-for="(articlesList ,index) in articles" :key="index" @click="articleIndexon(index)">
                      <div class="class_car_img">
                        <img src="/static/img/indexCarTree.jpg" alt="">
                      </div>
                      <div class="class_car_font">
                        <p class="class_car_title">{{articlesList.title}}</p>
                        <p class="class_car_box">{{articlesList.profile}}</p>
                        <key-wrod :keyword="articlesList.label_name"></key-wrod>
                        <p class="car_Font_color top_font"><MerchantModalDiv :name="articlesList.uid"></MerchantModalDiv> · {{articlesList.view_num}} 次浏览 · {{articlesList.created_at}}</p>
                      </div>
                    </li>
                  </ul>
              </div>
            ...
            <el-tab-pane name="fourth">
              <p slot="label" class="car_title_btm">
                <span>逛一逛</span>
                <router-link to="/home/videoDetail"><i class="icon icon-ziyuan45"></i></router-link>
              </p>
                ...
                <ul>
                   <li @click="VideoIndexClick(index)" v-for="(videoList ,index) in videos" :key="index">
                     <div class="car_section_img">
                       <video :id="'video'+index" v-show="index == videoPlay"  :src="request.picUrl+videoList.video_url" preload @mouseenter="playVideoComt(index)"></video>
                     ...
                     <p class="car_Font_color top_font"><!--{{videoList.nickname}}--><UserModalDiv></UserModalDiv></p>
...
export default {
    ...
    created(){
        this.$ajax.post(this.request.axiosPort+this.request.axiosApi+'index/intelligentlist',{
        })
        .then((res) => {
          console.log(res)
