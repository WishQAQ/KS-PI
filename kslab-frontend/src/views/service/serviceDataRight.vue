  ...
  <div class="service_box_data">
    <div class="service_box_DetailsVideo">
      <el-tabs v-model="activeNames" @tab-click="handleClick">
        <el-tab-pane label="相关服务" name="first">
          <ul class="service_right_body">
             <li v-for="(item, index) in rightData" style="cursor: pointer" :key="index" @click="handleService(item.service_id)">
               <div>
                  <img :src="handleImg(item.images[0].path)" alt="">
               </div>
               <div>
                 ...
                 <div class="service_right_bottom">
                   ...
                   <p class="right_person">{{item.signup_sum?item.signup_sum:'0'}}人已报名</p>
                 </div>
               </div>
             </li>
          </ul>
        ...
        <el-tab-pane label="相关视屏" name="second">
          <ul class="service_right_body">
             <li style="cursor: pointer" @click="handleJumpToVideo(item.video_id)" v-for="(item, index) in videoData" :key="index">
               <div>
...
export default {
  ...
  watch: {
    headerData: {
      handler(newData, oldData) {
        console.log(newData);
        this.getContactService(newData.title, newData.tagsname, newData.service_id);
        this.getContactVideo(newData.title, newData.service_id, newData.category_id);
      }
    }
  },
  mounted() {
  ...
  methods:{
     ...
     * @param service_id
     */
    getContactService(title,tags,service_id) {
      this.HttpClient.post('/services/relatedServices',{
        title: title,
        tags: tags,
     ...
     * @param category_id
     */
    getContactVideo(title, uid, category_id) {
      this.HttpClient.post('/services/relatedServicesVideo',{
        title: title,
        uid: uid,
    ...
    handleService(id) {
      this.$emit('getServiceId',id)
    },
    handleJumpToVideo(id) {
      this.$router.push({
