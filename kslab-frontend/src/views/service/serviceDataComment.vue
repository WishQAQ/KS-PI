                ...
                <div>
                  ...
                  <ul class="service_img_block">
                    <li class="img_content_commit" v-for="(item1, index) in item.attachment" :key="index">
                      <img class="commit_imgs" :src="handleImg(item1.path)" alt="">
                    </li>
                  </ul>
...
export default {
  ...
  methods: {
     ...
     */
    getComments(param) {
      this.service_id = param.source_id;
      HttpClient.post('/services/getAnswer',param).then(res =>{
        console.log(res)
        if(res.data.code === 200) {
          this.commitDetail = res.data.data.data;
    ...
    handleBeforeUpload(file) {
      ...
      if(file.size/1024<=1024 && (file.type === 'image/jpg' || file.type === 'image/jpeg' || file.type === 'image/png')){
        ...
        this.$ajax.post(this.request.axiosPort+this.request.axiosApi+'services/answersAttachment',FromData)
          .then((res) => {
            //console.log(res)
            if(res.data.code === 200){
     ...
     */
    handleCommit() {
      console.log(this.getRate,this.service_id)
      const commit = {
        token: window.localStorage.getItem('token'),
        source_id: this.service_id,
        content: this.commitInput,
        attachment: this.serialnum,
        score: this.getRate * 2
      };
      HttpClient.post('/services/answers',commit).then(res=>{
        //console.log(res)
        if(res.data.code === 200) {
    ...
    handlePageChange(page) {
      //console.log(page,this.service_id)
      const param = {
        page: page,
        size: this.size,
        source_id: this.$store.state.serviceDetail.serviceDetail.service_id,
        token: window.localStorage.getItem('token')
      };
      this.getComments(param);
    },
