  ...
  <div class="uploading_main_item uploading_main_type">
    ...
    <div class="main_type_list">
      <ul class="figure_list">
        <li class="list_item" :class="{active:index===checkedIndex}" v-for="(tmp,index) in serviceType" :key="index"
            @click.stop="checkedType(tmp.name,tmp.id,index,tmp.menu_id)">
          <div class="type_list_content">
            <div class="type_content_img_box"><i class="icon" :class="'icon-qiche'"></i></div>
            <div class="type_content_txt">
              <p class="content_txt_title">{{tmp.name}}</p>
            </div>
          ...
          <div class="type_list_select_box">
            ...
            <transition name="spread">
              <ul class="type_list_select" v-if="index===checkedIndex  && showSelect">
                <li class="type_select_item" v-for="(option,i) in tmp.child" :key="i"
                    @click.stop="selectedOption(option.name,option.id, option.menu_id)">
                  {{option.name}}
...
<script>
  export default {
    ...
    methods: {
      checkedType(type, id, index, menu_id) {//大类点击事件
        console.log(11111)
        ...
        this.$store.dispatch('getServiceUploadData',{category_id: menu_id}).then(res=>{
          console.log(this.$store.state.serviceUpload.serviceUploadData)
        });
      },
      selectedOption(val,id, menu_id) {//小类点击事件
