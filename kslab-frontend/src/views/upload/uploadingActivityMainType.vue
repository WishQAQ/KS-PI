    ...
    <div class="main_type_list">
      <ul class="figure_list">
        <li class="list_item" :class="{active:index===checkedIndex}" v-for="(tmp,index) in serviceType" :key="index"
            @click.stop="checkedType(tmp.name,tmp.id,index)">
            ...
            <transition name="spread">
              <ul class="type_list_select" v-if="index===checkedIndex  && showSelect">
                <li class="type_select_item" v-for="(option,i) in tmp.child" :key="i"
                    @click.stop="selectedOption(option.name,option.id)">
                  {{option.name}}
...
<script>
  export default {
    ...
    methods: {
      checkedType(type, id, index) {//大类点击事件
        ...
        this.category_id = id;
        this.$store.dispatch('getActivityUploadData',{category_id: id}).then(res=>{
          console.log(this.$store.state.activityUpload.activityUploadData)
        });
      },
      selectedOption(val,id) {//小类点击事件
