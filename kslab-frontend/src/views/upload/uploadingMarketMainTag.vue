    ...
    <div class="main_tag_add">
      <div class="main_tag_add_box" :class="{active: dynamicTags&&dynamicTags.length>0}">
        ...
        <div class="main_tag_list">
          <i class="el-icon-circle-plus" :class="{active:dynamicTags&&dynamicTags.length>0}" @click="handleInputConfirm"></i>
          <el-tag
            :key="tag"
  ...
  export default {
    ...
    watch: {
      marketTagData: {
        handler(newData, oldData) {
          ...
          this.dynamicTags = newData.tagsname
        },
        deep: true
      },
      inputValue: function () {
    ...
    methods: {
      handleClose(tag) {
        ...
        this.$store.dispatch('getMarketUploadData', marketData).then(res => {
