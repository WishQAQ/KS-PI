<template>
   <div class="ArticleBestAuthor">
           ...
           <div class="content_header">
             <div class="content_header_search">
               <el-input placeholder="昵称搜索" v-model="nickname" @keyup.13.native="getAuthorList">
                 ...
                 <el-select class="select_type" clearable v-model="userType" placeholder="全部" @change="getAuthorList">
                     <el-option
     ...
     <el-dialog
       ...
       <span slot="title" class="dataDialog_title"><i class="iconfont icon-image"></i>数据</span>
       <div class="dataDialog_box">
         ...
         <div class="dataDialog_list">
           ...
           <div class="dataDialog_list_main">{{averageViews}}</div>
         </div>

         <div class="dataDialog_list">
           <div class="dataDialog_list_header">平均点赞数</div>
           <div class="dataDialog_list_main">{{averageLikes}}</div>
         </div>

         <div class="dataDialog_list">
           <div class="dataDialog_list_header">平均讨论次数</div>
           <div class="dataDialog_list_main">{{averageComments}}</div>
         </div>

         <div class="dataDialog_list">
           <div class="dataDialog_list_header">平均字数</div>
           <div class="dataDialog_list_main">{{averageWords}}</div>
         </div>
  ...
  export default {
    ...
    methods:{
        ...
        getAuthorList(){
            ...
            this.HttpClient.post('/admin/users/ranking',parameters)
                .then(res=>{
                    console.log(res);
        ...
        pandect(item){
            ...
            this.averageViews=(Number(item.article_view)/this.articleTotal).toFixed(2);
            this.averageLikes=(Number(item.article_like)/this.articleTotal).toFixed(2);
            this.averageComments=(Number(item.article_comment)/this.articleTotal).toFixed(2);
            this.averageWords=(Number(item.word_num)/this.articleTotal).toFixed(2);
        },
        //分页
        currentChange(p){
            this.currentPage=p;
