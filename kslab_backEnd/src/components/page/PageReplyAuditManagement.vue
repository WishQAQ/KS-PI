...
export default {
    ...
    data: function() {	
        return {
            ...
            id: 2,
            name: '回复审核管理',
            urls: '/index/PageReplyAuditManagement',
            icon: 'icon-home'
          }],
          modifyVisible:false,
          name:'',//驳回类别名称
    ...
    methods: {
        //获取驳回类别列表
        getRefuseList(){
            this.HttpClient.post('/admin/webReview/getList',{type:'comments'})
                .then(res=>{
                    console.log(res);
        ...
        //新增驳回类别
        addNewRefuse(){
            this.HttpClient.post('/admin/webReview/create',{name:this.name,type:'comments'})
                .then(res=>{
                    console.log(res);
                    if(res.data.code===200){
                        this.name='';
