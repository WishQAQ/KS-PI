<template>
    <div>
        ...
        <el-dialog :visible.sync="modifyVisible" width="420px">
            ...
            <div class="btn">
                <el-button @click="modifyVisible=true">取消</el-button>
...
export default {
    ...
    data: function() {	
        return {
            ...
            id: 2,
            name: '活动&服务驳回类别',
            urls: '/index/PageDismissCategory',
            icon: 'icon-home'
          }],
          modifyVisible:false,//修改框
          name:'',//驳回类别名称
          refuseList:[],//驳回类别列表
          id:'',//驳回类别id
          newName:'',//修改类别名称
    ...
    methods: {
        ...
        //新增驳回类别
        addNewRefuse(){
            this.HttpClient.post('/admin/webReview/create',{name:this.name,type:'active'})
                .then(res=>{
                    console.log(res);
                    if(res.data.code===200){
                        this.name='';
        ...
        confirmModify(){
            if(this.newName){
                this.HttpClient.post('/admin/webReview/edit',{id:this.id,name:this.newName,type:'active'})
                    .then(res=>{
                        console.log(res);
                        if(res.data.code===200){
                            this.$message.success(res.data.msg);
