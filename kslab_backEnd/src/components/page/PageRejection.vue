...
export default {
    ...
    methods: {
        ...
        //新增驳回类别
        addNewRefuse(){
            this.HttpClient.post('/admin/webReview/create',{name:this.name,type:'upload'})
                .then(res=>{
                    console.log(res);
                    if(res.data.code===200){
                        this.name='';
        ...
        confirmModify(){
            if(this.newName){
                this.HttpClient.post('/admin/webReview/edit',{id:this.id,name:this.newName,type:'upload'})
                    .then(res=>{
                        console.log(res);
                        if(res.data.code===200){
                            this.$message.success(res.data.msg);
